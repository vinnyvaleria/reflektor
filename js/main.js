		...
		for (let row = 0; row < mapArray.length; row++) {
			for (let col = 0; col < mapArray[row].length; col++) {
				...
				if (mapArray[row][col] === 0) {
					gridType.querySelector(`#${gridName}-${count}`).textContent = "";
				}
				...
				if (mapArray[row][col] === 2) {
					...
					if (gridName === "main") {
						gridType.querySelector(`#${gridName}-${count}`).textContent = "P";
						playerID = count;
					}
					if (gridName === "sec") {
						gridType.querySelector(`#${gridName}-${count}`).textContent = "NPC";
						mirroredID = count;
					}
				}
				// update content to X to indicate goal when value is 3
				if (mapArray[row][col] === 3) {
					gridType.querySelector(`#${gridName}-${count}`).classList.add("goal");
					gridType.querySelector(`#${gridName}-${count}`).textContent = "X";
				}
				count++;
			}
		}
